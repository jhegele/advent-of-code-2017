...
class Grid(object):
    ...
    def add_point(self, target):
        if len(self.points) == 0:
            ...
            value = sum([pt.value for pt in self.points if abs(pt.x - next_x) <= 1 and abs(pt.y - next_y) <= 1])
            self.last_move = move
            self.current_sequence.append(move)
            self.points.append(Point(value, next_x, next_y))
            self.done = (value > target)
...
grid = Grid()
while not grid.done:
    grid.add_point(puzzle_input)

print('{}'.format(grid.points[-1].value))
